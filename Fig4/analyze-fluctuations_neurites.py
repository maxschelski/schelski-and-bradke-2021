...
def move_further(current_point,current_pos,all_fluctuations,
                 all_fluctuation_points,average_intensities,
                 rel_ex_type,min_diff_for_cycle):
    #check minima after if one of them is different enough
    if rel_ex_type == "max":
        points_after = all_minima[np.where(all_minima>current_point)]
    else:
        points_after = all_maxima[np.where(all_maxima>current_point)]
#    print(current_point)
    point_val = average_intensities[current_point]
    if len(points_after) > 0:
        for next_point in points_after:
                next_point_val = average_intensities[next_point]
#            print("min: {} - current_pos: {}".format(minimum,current_pos))
                if len(all_fluctuations) == 0:
                    if rel_ex_type == "max":
                        if next_point_val > point_val:
                            rel_ex_type = "min"
                            current_point = all_minima[np.where(all_minima > current_point)[0][0]]
                            current_point, all_fluctuations, all_fluctuation_points = move_further(current_point,current_pos,all_fluctuations,all_fluctuation_points,average_intensities,rel_ex_type,min_diff_for_cycle)
                            break
                    else:
                        if next_point_val < point_val:
                            rel_ex_type = "max"
                            current_point = all_maxima[np.where(all_maxima > current_point)[0][0]]
                            current_point, all_fluctuations, all_fluctuation_points = move_further(current_point,current_pos,all_fluctuations,all_fluctuation_points,average_intensities,rel_ex_type,min_diff_for_cycle)
                            break
                #check points between for lower or higher values than current extrema
                if rel_ex_type == "max":
                    points_between = all_maxima[np.where((all_maxima > current_point) & (all_maxima < next_point))]
                    if len(points_between) > 0:
                        points_between_vals = average_intensities[points_between]
                        alternative_point = points_between[np.where(points_between_vals == max(points_between_vals))[0][0]]
                        if max(points_between_vals) > point_val:
                            if len(all_fluctuations) > 0:
                                all_fluctuations[-1] = all_fluctuations[-1] + abs(average_intensities[alternative_point] - average_intensities[current_point])
                            current_point = alternative_point
                            
                else:
                    points_between = all_minima[np.where((all_minima > current_point) & (all_minima < next_point))]
                    if len(points_between) > 0:
                        points_between_vals = average_intensities[points_between]
                        alternative_point = points_between[np.where(points_between_vals == min(points_between_vals))[0][0]]
                        if min(points_between_vals) < point_val:
                            if len(all_fluctuations) > 0:
                                all_fluctuations[-1] = all_fluctuations[-1] + abs(average_intensities[alternative_point] - average_intensities[current_point])
                            current_point = alternative_point
                        
#                        print("max before: {} - max max: {}".format(maximum,maximum_max))
                point_val = average_intensities[current_point]
    
                next_point_val = average_intensities[next_point]
                diff = abs(point_val - next_point_val)
#                print("now: {} / {} - next: {} / {}".format(current_point,point_val,next_point,next_point_val))
                if (diff > min_diff_for_cycle) :
                    current_point = next_point
                    all_fluctuations.append(diff)
                    all_fluctuation_points.append(next_point)
                    if rel_ex_type == "max":
                        rel_ex_type = "min"
                    else:
                        rel_ex_type = "max"
                    current_point, all_fluctuations, all_fluctuation_points = move_further(current_point,current_pos,all_fluctuations,all_fluctuation_points,average_intensities,rel_ex_type,min_diff_for_cycle)
                    break
    return current_point, all_fluctuations, all_fluctuation_points
