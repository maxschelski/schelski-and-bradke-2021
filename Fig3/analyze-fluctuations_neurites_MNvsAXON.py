...
def classify_stages_from_axon_formation(all_data, outgrowth_time_data):
    ...
    for neuron in all_neurons.values:
        if tuple(neuron) in outgrowth_idx:
            ...
            growth_time_st2 = growth_time - 120
...
def analyze_fluctuations(all_data, int_column, unit_columns, all_thresholds,
    ...
    data = pd.DataFrame(columns=("date", "neuron",
                                 "stage", "neurite",
                                 "frames", "cycles",
                                 "cycles/h", "cycle_amplitude",
                                 "pos_amplitude", "neg_amplitude",
                                 "time_for_increase", "time_for_decrease",
                                 "speed_increase", "speed_decrease",
                                 "first_pos_amplitude", "first_neg_amplitude",
                                 "first_time_for_increase", "first_time_for_decrease",
                                 "first_speed_increase", "first_speed_decrease",
                                 "cycle_threshold"))
    ...
    for one_index in all_units.values:
        ...
        one_branch_mean_int = one_branch['avInt_smooth'].mean()
            ...
            if len(mean_ints) > 0:
                ...
                if (len(all_minima) > 0) & (len(all_maxima) > 0):
                    ...
                    for min_diff_for_cycle in all_thresholds:
                        ...
                        all_flucts = []
                        all_first_flucts = []
                        all_first_fluct_points = [start_point]
                        #only continue, if minimum before was found 
                        #(otherwise maximum could not be chained 
                        # to last minimum)
                        results = move_further(start_point,
                                               all_flucts, all_first_flucts ,
                                               all_minima, all_maxima, 
                                               all_fluct_points, all_first_fluct_points,
                                               mean_ints, 
                                               rel_ex_type, min_diff_for_cycle)
                        current_point = results[0]
                        all_flucts = results[1]
                        all_first_flucts = results[2]
                        all_fluct_points = results[3]
                        all_first_fluct_points = results[4]
                        ...
                        #calculate the speed of increase and the speed of decrease
                        #first calculate the time differences
                        ...
                        fluct_time_points = one_branch.iloc[all_fluct_points].reset_index()["time"].values
                        time_differences = fluct_time_points[1:] - fluct_time_points[:-1]
                        time_difference_pos_amp = time_differences[pos_amplitudes_idxs]
                        time_difference_neg_amp = time_differences[neg_amplitudes_idxs]
                        
                        increase_speeds = pos_amplitudes / time_difference_pos_amp
                        decrease_speeds = neg_amplitudes / time_difference_neg_amp
                        
                        #calculate the amplitudes of the increases and decreases
                        #if just considering the first point reached that changed enough
                        #for the fluctuation threshold
                        first_fluct_points_int = mean_ints[all_first_fluct_points]
                        #the start point is the final fluctuation point
                        #the end point is the first point changing more than threshold
                        first_fluct_points_diff = first_fluct_points_int[1:] - fluct_points_int[:-1]
                        first_pos_amplitudes = first_fluct_points_diff[pos_amplitudes_idxs]
                        first_neg_amplitudes = first_fluct_points_diff[neg_amplitudes_idxs]
                        
                        first_fluct_time_points = one_branch.iloc[all_first_fluct_points].reset_index()["time"].values
                        first_time_differences = first_fluct_time_points[1:] - fluct_time_points[:-1]
                        first_time_difference_pos_amp = first_time_differences[pos_amplitudes_idxs]
                        first_time_difference_neg_amp = first_time_differences[neg_amplitudes_idxs]
                        
                        first_increase_speeds = first_pos_amplitudes / first_time_difference_pos_amp
                        first_decrease_speeds = first_neg_amplitudes / first_time_difference_neg_amp
                    
                        new_row = []
                        new_row.append(one_index[0])
                        new_row.append(one_index[1])
                        new_row.append(stage)
                        new_row.append(one_index[2])
                        ...
                        new_row.append(np.mean(time_difference_pos_amp))
                        new_row.append(np.mean(time_difference_neg_amp))
                        new_row.append(np.mean(increase_speeds))
                        new_row.append(np.mean(decrease_speeds))
                        new_row.append(np.mean(first_pos_amplitudes))
                        new_row.append(np.mean(first_neg_amplitudes))
                        new_row.append(np.mean(first_time_difference_pos_amp))
                        new_row.append(np.mean(first_time_difference_neg_amp))
                        new_row.append(np.mean(first_increase_speeds))
                        new_row.append(np.mean(first_decrease_speeds))
...
def move_further(current_point, all_flucts,
                 ...
                 rel_ex_type, min_diff_for_cycle):
    """
    :param all fluct_points: positions in array that is part of final fluctuation
    :param all_first_fluct_points: positions in array at which the increase or 
                                    decrease was first observed
                                    (although it might go even higher or lower later)
    """
    ...
    for next_point in points_after:
        ...
        #since the new found value is in the other array
        #the rel_ex_type must be switched
        if len(all_flucts) == 0:
            if rel_ex_type == "max":
                if next_point_val > point_val:
                    ...
                    all_first_fluct_points = [current_point]
        ...
        if (diff > min_diff_for_cycle) :
            ...
            all_first_flucts.append(diff)
            
            all_fluct_points.append(next_point)
            all_first_fluct_points.append(next_point)
...
file_name_outgrowth_time = "axon_outgrowth_time.csv"


#get data of at which time the axon grew out from this script:
#C:\Users\Maxsc\Nextcloud\13PROGRAMSandSCRIPTS\Python\MT-fluctuations
#\axon\get_timepoint_of_axon_growth.py
outgrowth_time_data = pd.read_csv(os.path.join(input_path, 
                                               file_name_outgrowth_time))
...
neurite_cols = [*neuron_cols, "neurite"]
outgrowth_time_data.set_index(neuron_cols, inplace=True)

#for each neuron, add information 
#about at which time its in stage 2 and in which time its in stage 3
all_data = classify_stages_from_axon_formation(all_data, outgrowth_time_data)

# file_name = file_name.replace(".csv", "_stages.csv")
# all_data.to_csv(os.path.join(input_path, file_name))

unit_cols = [*neurite_cols, "stage"]
...
ints_norm = all_data.groupby(norm_cols)["intensity"].transform(lambda x: x / x.mean())
all_data.loc[:,"int_norm"] = ints_norm

ints_norm = all_data.groupby(norm_cols)["MT-RFmed_neg"].transform(lambda x: x / x.mean())
all_data.loc[:,"MT-RFmed_neg_norm"] = ints_norm


ints_norm = all_data.groupby(norm_cols)["MT-RFmed"].transform(lambda x: x / x.mean())
all_data.loc[:,"MTR-Fmed_norm"] = ints_norm

all_data.to_csv(os.path.join(input_path, file_name.replace(".csv","_norm.csv")))

# print(all_data.groupby(unit_cols)["time"].apply(lambda x: x.iloc[-1] - x.iloc[0]))
# print(all_data.groupby(norm_cols)["MT-RFmed"].transform(lambda x: x / x.mean()))
# print(all_data.groupby(unit_cols)["int_norm"].apply(lambda x: x.iloc[-1] - x.iloc[0]))
...
#add column with information whether neurite is axon (neurite == 0)
fluct_data["axon"] = 0
fluct_data.loc[fluct_data["neurite"] == 0, "axon"] = 1

file_name = file_name.replace(".csv","_fluctuations.csv")
data_path = os.path.join(input_path, file_name)
# fluct_data.to_csv(data_path)
fluct_data = pd.read_csv(data_path)
print(all_data["time"].head(20))
all_data = all_data.reset_index().set_index(unit_cols)
print(all_data["intensity_norm_smooth"].head(20))
all_data["int_gain"] = all_data.groupby(unit_cols)["intensity_norm_smooth"].apply(lambda x: x.iloc[-6] - x.iloc[5])
print(all_data["int_gain"])
all_data["length_gain"] = all_data.groupby(unit_cols)["length"].apply(lambda x: x.iloc[-1] - x.iloc[0])
print(all_data["length_gain"])
all_data["time_diff"] = all_data.groupby(unit_cols)["time"].apply(lambda x: x.iloc[-1] - x.iloc[0])

all_data["int_gain_rate"] = all_data["int_gain"] / all_data["time_diff"]
all_data["length_gain_rate"] = all_data["length_gain"] / all_data["time_diff"]

#add MTRF speed data to fluct_data
data_means = all_data.groupby(unit_cols).mean()
fluct_data.set_index(unit_cols, inplace=True)
for column in data_means.columns:
    fluct_data[column] = np.nan

for unit_idx in data_means.index.values:
    unit_idx = tuple(unit_idx)
    unit_means = data_means.loc[unit_idx] 
    for column in data_means.columns:
        fluct_data.loc[unit_idx, column] = unit_means[column]



file_name = file_name.replace(".csv","_MTRF.csv")
data_path = os.path.join(input_path, file_name)
fluct_data.to_csv(data_path)